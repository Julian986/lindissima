...
import { isLikelyWhatsappNumber, canonicalPhoneDigitsAR, customerPhoneDigitsQueryValues } from "@/lib/customer/phone-canonical-ar";
...
export async function POST(request: Request) {
  ...
  if (!isLikelyWhatsappNumber(phone)) {
    return NextResponse.json(
      { error: "Ingresá un número de WhatsApp válido (10 a 15 dígitos)." },
      { status: 400 },
    );
  }
  ...
  // Validar que el número tenga al menos una reserva antes de emitir la cookie.
  let customerName: string | null = null;
  try {
    const db = await getDb();
    const keys = customerPhoneDigitsQueryValues(digits);

    // Busca primero por campo normalizado (registros nuevos), luego por fallback
    // en customerPhone (registros creados antes de la normalización).
      ...
      .findOne<{ customerName?: string }>(
        {
          $or: [
            ...
            // Fallback para reservas viejas sin customerPhoneDigits: últimos 8 dígitos
            { customerPhone: { $regex: digits.slice(-8) } },
          ],
        },
